...
const SERVER_INSTRUCTIONS = [
  ...
  'Call semantic_search when you want raw retrieval snippets, context_bundle for a structured file packet, or graph_neighbors to expand through the GraphRAG index; each call updates hit counters so eviction keeps frequently used data resident.',
  'Set the INDEX_MCP_BUDGET_TOKENS environment variable or pass budgetTokens/context_bundle arguments to cap tokens and avoid overloading downstream LLM context, and enable autoEvict on ingest when you need the server to trim the database automatically.',
  'Use repository_timeline to review recent commits, merges, and churn so plan-of-action agents can factor fresh changes into their reasoning; diffs ship by default so code changes stay visible.',
  'If ingest_codebase reports "UNIQUE constraint failed: code_graph_nodes...", rerun it with graph.enabled set to false while the duplicate-node bug is addressed, and always respect ignore files so repeated ingests stay clean.'
].join(' ');
