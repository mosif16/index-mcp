...
- The walker hashes files, respects a configurable max size, and stores metadata and (optionally) file contents in the `files` table. Embeddings are produced through a pluggable backend (FastEmbed ONNX by default, Candle sentence-transformers when `embedding.backend="candle"`, and a lightweight `mock` backend compiled only for tests) with per-model caching, then written to `file_chunks` alongside byte/line ranges and hit counters. When embeddings are present the ingest pass also emits a `.ann/<basename>.hnsw.*` index plus `.ids` mapping for HNSW-based ANN search ([ingest.rs:57-206,640-765]).
