...
- **Backends**: `embedding.backend` selects the embedding runtime. `fastembed` (default) loads ONNX weights such as `Xenova/all-MiniLM-L6-v2` and its quantized variants; `candle` boots CPU-only sentence-transformer pipelines built on Candle. A `mock` backend is compiled behind `cfg(test)` for deterministic integration tests and should not be used in production. All backends report `embedding_backend`, `embedding_dimension`, and `embedding_quantized` in `IngestResponse` and the SQLite `meta` table.
